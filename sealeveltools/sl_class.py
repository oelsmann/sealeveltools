...
class sl(slt_object):
    ...
    def plot(self,vector=False,return_fig=False,**kwargs):   
        ...
        if return_fig:
            return plt_xr_map(self.data,return_fig=return_fig,**kwargs)
        else:
    ...
    def AC(self,data2,s_method='nearest',limit=100.,t_method='time',dropna=True,**kwargs):
        """
        compute AC between datasets
        ...
        if data2.typ =='pd' or data2.typ =='xr':    
            if data2.typ =='pd':
                ...
                diff_ = self.data-data2.data
                diff_annual = sl(diff_).yearmon().data
                diff_annual_amp = (diff_annual.max(dim='time')-diff_annual.min(dim='time'))

            data_AC=diff_annual_amp.values.reshape(self.shape[1:])
            self.data[0,...]=0
            new_data=self.data[0,...]+data_AC
            new_data.attrs={'standard_name': 'AC', 'long_name': self.var+' AC. '+data2.var,
            ...
            for var in list(self.data.keys())[:]:
                sl_list.append(sl(self.data[var]).AC(data2).data)
    ...
    def zoi(self,other,output='zoi_series',opt='cor',how='rel',level=0.8,avg='mean',
            limit=100.,both=False,detrend_deseason=False,**kwargs):
        """
        a) maps two datasets:
        ...
        e) uses external data (detrended/deseasoned to compute more statistics)
        ...
        if detrend_deseason:
            """
            use deseasoned dataset for other 
            and kwargs['self_adjusted'] for self (deseasoned)

            """
            self,other_2 = self.remap(other,both=True,s_method='zoi',limit=limit)
            self_or=copy.deepcopy(self.data) 
            self = sl(kwargs['self_adjusted'])
        ...
        if opt=='out':
            ...
            self_cor=getattr(sl(self), opt)(other,dropna=False) # e.g. opt='cor' or 'rms'
        if output=='zoi_stats':
            if both:
    ...
    def detrend(self,monthly=False,de_season=True,semi=False,weights=None,**kwargs):
        """
        ...
        **kwargs ={'de_season' : 'False','semi' : 'False'}
        ...
        if kwargs['de_season']:
            factor=1.
            print('subtract trend + annual cycle (no semi)')
        else:
            factor=0.            
            print('only subtract trend')
        
        if len(shp)>2:
            detrend_arr=annual_cycle(self.flat_y_numeric,out['offset'].values[0,:,:,np.newaxis],
                     out['trend'].values[0,:,:,np.newaxis],
                     out['acos'].values[0,:,:,np.newaxis]*factor,
                     out['asin'].values[0,:,:,np.newaxis]*factor)        
            ...
            detrend_arr=annual_cycle(self.flat_y_numeric,out['offset'].values[:,:,np.newaxis],
                     ...
                     out['acos'].values[:,:,np.newaxis]*factor,
                     out['asin'].values[:,:,np.newaxis]*factor)             
    ...
    def trend(self,how='leastsq',de_season=True,semi=False,
                       ...
                       model='AR1',hector_out=False,monthly=False,weights=None,var=''):
        """
        ...
        weights: name of uncertainties in data, default None
        ...
        kwargs={'de_season':de_season,'semi':semi,
                ...
                'hector':hector,'model':model,'hector_out':hector_out,
                        'monthly':monthly,'weights':weights,'var':var}
    ...
    def yearmon(self,how='mean',op=None):
        ...
        if self.typ == 'xr':
            ...
            all_=[]
            for var in list(self.data.keys()):
                out=copy.deepcopy(self.data[var])
                all_.append(yearmon_sub(out))
            out=xr.merge(all_)
        return sl(out)
    ...
    def grid_pd(self,grid_size,other=None,opt='cartesian'):
        """
        ...
        flat_x=self.flat_x
        grid_factor = 1/grid_size
        
        max_lat=np.round(np.max(lat)*grid_factor)/grid_factor
        min_lat=np.round(np.min(lat)*grid_factor)/grid_factor
        max_lon=np.round(np.max(lon)*grid_factor)/grid_factor
        min_lon=np.round(np.min(lon)*grid_factor)/grid_factor
        ...
        elif opt=='linear' and other != None:

            lat_x=sl(other).flat(kind='lat')
            lon_y=sl(other).flat(kind='lon')
            grid_x, grid_y = lat_x,lon_y#np.meshgrid(lat_x,lon_y)
            max_leng=np.max([len(lat_x),flat_x.shape[-1]])
        ...
        if opt=='cartesian':
            ...
            if len(dat.shape)>1:
                self=reconstruct_irreg_data(self,dat.loc[:,:len(lat_x)-1].values,lat_x, lon_y,time_=time_)
            else:
                self=reconstruct_irreg_data(self,dat.loc[:len(lat_x)-1].values,lat_x, lon_y,time_=time_)

        return self
    
    def select_and_reorganize_data(self):
        """
        selects frist variable restructures 
        data if time is not first dimension
    ...
    def couple(self,other,limit=100.,s_method='nearest',swap=True,**kwargs):
        ...
        if other.typ != 'float':
            ...
            if self.typ=='xra': #to be fixed, add loop
                self.data=self.select_and_reorganize_data()
    ...
    def remap(self,other=None,both=False,rmptime=False,grid_size=.5,s_method='nearest',
        ...
        s_method: str, default 'nearest'
            remapping method: 
                'nearest','median','mean': look up the $s_method within a limit and match
                'zoi': look up the highest correlated or lowest rms values within limit and percentage
                'linear': linear interpolation on other/regridding
                'sfltr': spatial filter with radius = limit and weights = 'trend_err' or 'trend_un' (naming convention)
    ...
    def match_xr(self,data2,rmptime=True,swap=True,**kwargs):
        ...
        """
        if data2.typ=='xra':
            data2.data=data2.select_and_reorganize_data()
...
#
                ...
                elif kwargs['s_method']=='linear':
                    # linear interpolation of the datasets:
                    self=self.grid_pd(5,other=data2,opt='linear')
                    _swap=False
                else:
                    ...
                    if kwargs['s_method'] =='nearest':
                        ...
                        flat_x2[...,dist.index]=data2.flat_x[...,dist.index]     # new structured data for second dataset
                        flat_xnew=self.flat_x[...,dist.idx.values.astype(int)]   # new structured data for self
