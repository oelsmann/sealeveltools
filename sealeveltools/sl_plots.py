...
def plt_xr_map(ds,var='all_',time=[0],ranges=[[0,0]],units=[''],edgecolors=[None],
               ...
               return_fig=False,lsize=0.15,prj=ccrs.PlateCarree(),cool_scatter=False,
               land_color='gainsboro',xpl=0,ypl=0,cbar_last=False,kind=[],
               v_factor=10.,alpha=1.,scale=None,**kwargs):
    ...
    land_color:       color of land to plot
    xpl:              manually set subplot structure as number of plots in x-directions and
    ypl:              y-direction
    cbar_last:        only add cbar at the last plot
    v_factor:         controls density of vector plots
    kind:             kind of plot: scatter, contour ...
    alpha:            opacity of scatter plots
    scale:            scale scatter plots dots by data, None, linear or sqrt
    
    **kwargs:
    
    """
    global labels_on,cb_pos,cb_siz,zorder_oni,cbar_lasti,v_factori,proj,draw_labels,scaler,cool_s,alpha_,scale_
    scaler=lsize
    draw_labels=False
    cb_siz=cb_size
    labels_on=labels
    ...
    land_50m = cfeature.NaturalEarthFeature('physical', 'land', '10m',
                                    edgecolor='face',
                                    facecolor=land_color)#'#f7f7f7')#cfeature.COLORS['land'])
    plt.ioff()
    ...
    plot_number,var,date_in,ranges,units,msizes,time_setter,edgecolors,overlay,multi_overlay,ax_index,sub_plot_index,kind=arrays_options(ds,var,time,ranges,units,msize,edgecolors,kind,extend)
    ...
    #    fig.colorbar(im,cax=cbar_ax,orientation='horizontal')
        ...
        for i in range(1,plot_number+1):
            ...
            if str(type(ds))=="<class 'list'>": # overlay plots
                ...
                if cbar_args['joint']:
                    cbarb=False
                if cbar_last and ii != plot_number:
                    cbarb=False                    
                if 'long_name' in ds_f.attrs:
                    title=ds_f.long_name        
                else:
...
def plt_xr_map_ax(ax,fig,data,lat,lon,land_50m,title,year,time_setter,range_in,unit,extend,lbl,
    ...
    if function=='scatter':
        ...
        if scale_!=None:
            if scale_=='linear':
                ...
                factor=np.std(data)
                s=abs((data-np.median(data))/factor)
                s[abs(data-np.median(data))<(0.5*factor)]=0.5
                msize_factor=s
                #s=s*factor
            elif scale_=='sqrt':
                factor=1
                s=np.sqrt(abs(data))
                s[abs(data)<0.5]=np.sqrt(0.5)
                msize_factor=s*factor
        else:
            msize_factor=1
        ...
        if cbarb:
            if cool_s:
                ax.scatter(lon,lat,c=data,s=msize*0.6*msize_factor,alpha=0.8,
                        vmin=minv,vmax=maxv,cmap=color_map,transform=ccrs.Geodetic(),zorder=5)

            im = ax.scatter(lon,lat,c=data,s=msize*msize_factor,edgecolor=edgecolor,alpha=alpha_,
                            vmin=minv,vmax=maxv,cmap=color_map,transform=ccrs.Geodetic(),zorder=0)
            
            
        else:
            ax.scatter(lon,lat,c=data,s=msize*msize_factor,edgecolor=edgecolor,alpha=alpha_,
                       vmin=minv,vmax=maxv,cmap=color_map,transform=ccrs.Geodetic(),zorder=0)
            ...
            if cbarb:
                ...
                im=ax.pcolormesh(lon,lat,data,vmin=minv,vmax=maxv,cmap=color_map,transform=proj,zorder=0)    #cmap=color_map,
            else:
                ax.pcolormesh(lon,lat,data,vmin=minv,vmax=maxv,cmap=color_map,transform=proj,zorder=0)
    
    elif function =='contour_lines':

            cbarb=False
            ax.contour(lon,lat,data,levels=0,colors='k',transform=proj,zorder=0)
            ...
            lon, lat, u, v=re_sample_data(lon, lat, u, v,factor=v_factori)
            print(range_in[0])
            ax.quiver(lon, lat, u, v, scale_units='inches',scale=range_in[0],transform=proj)
    ...
    if extend[0] != 9999:
        ax.set_extent(extend, crs=proj)
        xticks,yticks=plt_make_ticks(extend[2:])   
    else:
        xticks,yticks=plt_make_ticks(lat) 
        extend=[np.min(lon),np.max(lon),np.min(lat),np.max(lat)]
        ax.set_extent(extend, crs=proj)
        
    if function is not 'vector':
        gl=ax.gridlines(draw_labels=draw_labels,xlocs=xticks, ylocs=yticks, alpha=0.25)
        gl=gl_props(gl,scaler)
    #ax.set_title(title)
...
def make_map_data(ds,vari,date_ini,function):
    ...
    if vari=='vector':
        ...
        if len(lat) != len(lon) or (('lat' in ds_f.dims) & ('lon' in ds_f.dims)):
            """
            draw contour lines when we have a regular grid
...
def arrays_options(ds,var,time_in,ranges,units,msize,edgecolors,kind,extend):
    """
    ...
    if kind != []:
        kind = adjust_array_len(kind,plot_number)         
    ...
    return plot_number,var,time_in,ranges,units,msizes,time_setter,edgecolors,overlay,multi_overlay,ax_index,sub_plot_index,kind
